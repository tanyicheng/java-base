...
//            write("192.168.3.101",4002,1,new String[1],false);
            String[] read = read("127.0.0.1", 502, 1, 0, false);
            for (String s : read) {
    ...
    public static String[] read(String ip, int port, int startPos, int num, boolean swapHL) throws Exception {
        ...
        String readCmd = "000000000006010300000005".replaceAll(" ", "");
...
//        DataInputStream dis = new DataInputStream(socket.getInputStream());
        byte[] buffer = new byte[8]; //从开头到数据长度
        InputStream input = socket.getInputStream();
        int count = input.read(buffer,0,8);
        System.out.println(count);
//        dis.readFully(buffer);
//        String response = "D0 00 00 FF FF 03 00 0C 00 00 00 0C 00 00 00 00 00 00 00 00 00".replaceAll(" ", "");
        String response = HexUtil.bytesToHexString(buffer);//从开头到数据长度
        String[] results = null;
        System.out.println("response：" + response);

        buffer = new byte[11];
        input.read(buffer,0,11);
        response = HexUtil.bytesToHexString(buffer);//从开头到数据长度
